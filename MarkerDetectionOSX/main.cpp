...
int main (int argc, char * const argv[]) {
	...
	Buffer* buffer = new Buffer();      //new buffer
	EdgelDetector* edgelDetector = new EdgelDetector(); //new detector
	
    //debug setting
	edgelDetector->debugDrawLineSegments( true );
	edgelDetector->debugDrawPartialMergedLineSegments( true );
	edgelDetector->debugDrawMergedLineSegments( true );
	edgelDetector->debugDrawExtendedLineSegments( true );   //!!
	edgelDetector->debugDrawSectors( true );
	edgelDetector->debugDrawSectorGrids( true );
	...
	if( !useCamera ) {
		draw_image = cvLoadImage( "/Users/reinder/Desktop/test1.png" );
		if(!draw_image){
        ...
        // set the image buffer to detector
